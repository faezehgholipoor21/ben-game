    ...
    <div class="container-fluid">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">
                                سفارشات {{\App\Helper\GetOrderStatusTitle::get_order_status_title($order_status_id)}}</li>
                        </ol>
                    </nav>
                </div>
            </div>
        ...
        <div class="row clearfix">
            <div class="col-12">
                <div class="card">
                    <form method="post"
                          action="{{route('admin.separate_order_search' , ['order_status_id' => $order_status_id])}}"
                          class="card-body">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-12 col-sm-6  @if(!$searched) col-md-3 @else col-md-2 @endif mb-4">
                                ...
                                <select name="status" class="form-control">
                                    @foreach($order_status as $order)
                                        <option @if($order['id'] == $order_status_id) selected="selected"
                                                @endif value="{{$order['id']}}">{{$order['title']}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            @else
                                <div class="col-12 col-md-2 mb-4">
                                    <a href="{{route('admin.separate_orders' , ['order_status' => $order_status_id])}}"
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-custom spacing8">
                            ...
                            @if(!empty($my_order_list->all()))
                                @php
                                    $row = (($my_order_list->currentPage() - 1) * $my_order_list->perPage() ) + 1;
                                @endphp
                            @else
                                @php
                                    $row = 1;
